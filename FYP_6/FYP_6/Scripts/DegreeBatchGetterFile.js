$(document).ready(function () {
    ...
    $("#degree").change(function () {
        if ($("#degree").val() != "Please select") {
            var options = {};
            options.url = "/Employees/GetBatches";
            options.type = "POST";
    ...
    $("#batch").change(function () {
        if ($("#batch").val() != "Please select" && $("#batch").val() != null) {
            var options = {};
            options.url = "/Employees/GetSections";
            options.type = "POST";
    ...
    $(".batchChangeGenNewRoll").change(function () {
        if ($(".batchChangeGenNewRoll").val() != "Please select" && $(".batchChangeGenNewRoll").val() != null) {
            //alert("change");
            var options = {};
            options.url = "/Employees/GetNewRollNo";
            options.type = "POST";
            options.data = JSON.stringify({ batch: $(this).val() });
            options.dataType = "json";
            options.contentType = "application/json";
            options.success = function (newRoll)
            {
                //$('#aeRCHRESULT').html(section);
                $(".GenNewRoll").empty();
                //$("#section").append("<option value='Please select'>Please select</option>");
                $(".GenNewRoll").val(newRoll);

                //$("#section").prop("disabled", false);
            };
            options.error = function () { alert("Error retrieving Rollno!"); };
            $.ajax(options);
