...
namespace FYP_6.Models.Models_Logic
{
    public class BatchModel
    {
         ...
         public IEnumerable<Batch> getAllBatches(int? category, string searchBatches)
        {
            try
            {
                if (searchBatches != null && searchBatches != "")
                {
                    var getRecords = rc.Batches.Where(s => s.Status == category
                        &&s.BatchName.StartsWith(searchBatches)).OrderBy(s => s.BatchName).Select(s => s);
                    return getRecords;
                }
                else
                {
                    var getRecords = rc.Batches.Where(s => s.Status == category).OrderBy(s => s.BatchName).Select(s => s);
                    return getRecords;
                }
            }
            catch (Exception)
        ...
        public  string AddBatch(Batch batch, string degree, string section, string year)
        {
            //deg.LevelID = level;
            using (TransactionScope t=new TransactionScope())
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (batch.BatchName.Length>9)
                            {
                                return "Batch Name must be less than 9 characters";
                            }
        ...
        private  bool ValidateBatchName(string batch)
        {
        ...
        //    }
        //}
        public  IEnumerable<Batch> getAllSearchSpecificBatches(string search)
        {
        ...
        public  Batch UpdateBatchRec(Batch batchRec, Guid degID, Guid secID, Guid yearID)
        {
        ...
        public  string NewBatchFinalRegister(Batch bat, Batch_Subjects_Parts bps, IEnumerable<string> subj)
        {
        ...
        //    }
        //}
        public  List<Registeration> GetAllRelevantBatchStudents(string batch)
        {
            var getStudents = rc.Registerations.Where(s => s.BatchID == batch).OrderBy(s => s.Rollno).Select(s => s).ToList();
            return getStudents;
        }

        public  string DeleteBatchesRecords_BatchModelFunc(IEnumerable<Guid> deleteBatch)
        {
            if (deleteBatch != null)
            {
                using (TransactionScope t = new TransactionScope())
                {
                    try
                    {
                        ...
                        var getStudentProfilesForThatBatch = rc.Registerations.
                            Where(s => deleteBatch.Contains(s.Batch.BatchID) &&s.Status==1)
                    ...
                    {
                        t.Dispose();
                        return "Unable To Delete Batches Records! Plz Try Again!";
                    }
                }
            }
            else
        ...
        public  string AddBatch_Subj_PartRec(Guid degree, string batch, Guid section, Guid subjects, int part
        ...
        public  string UpdateBatch_Subj_PartRec(Guid degree, string batch, Guid section, int part)
        {
            using (TransactionScope t=new TransactionScope())
            {
                try
                {
                    ...
                    var getDegreeName = rc.Degree_Program.Where(s => s.ProgramID == degree).Select(s => s.Degree_ProgramName).FirstOrDefault();
                    var getSectionName = rc.Sections.Where(s => s.SectionID == section).Select(s => s.SectionName).FirstOrDefault();
                    var getFirsRegisteration = rc.Registerations.Where(s => s.BatchID == batch).Select(s => s).FirstOrDefault();

                    if (getFirsRegisteration==null)
                    {
                        return "No Students Found To Change Part!";    
                    }
                    int partForRegbatch = getFirsRegisteration.Part??1;
                    partForRegbatch++;
                    if (partForRegbatch >= 5)
                    {
                        return "The Batch " + batch + " has Completed its Degree Duration! ";
                    }
                    if (part!=partForRegbatch)
                    {
                        return "Plz Select Part: "+partForRegbatch+" For This Batch!";
                    }
                    ...
                    if (checkerForAlreadyPartPresentInAssignedSubjects == false)
                    {
                        ...
                        var getRelStd = rc.Registerations.Where(
                            ...
                            && s.Batch.SectionID == section
                            && s.Batch.DegreeProgram_ID == degree
                            && s.Status == 1 && s.Student_Profile.Status == 1);
                ...
                {
                    t.Dispose();
                    return "Unable To Change Part! Plz Try Again!";
                }   
            }
        }

        //Update Subjects in Batch_Subjects_Parts Table
        public  string SubjectAddToBPS(IEnumerable<Guid> subj, Batch batch,int part)
        {
            try
            {
        ...
        public  bool AssignNewSubjectsToBPS(List<Subject> CompleteSubjList,
        ...
        public  bool DeleteAllSubjectsToBPS(Batch batch ,int part)
        {
            using (TransactionScope t = new TransactionScope())
            {
                try
        ...
        public  IEnumerable<Degree_Subject> getSubjectsForEditing(Batch batch)
        {
